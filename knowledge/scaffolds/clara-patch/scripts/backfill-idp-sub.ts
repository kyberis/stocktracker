/**
 * One-shot migration: provision every Clara user in the trefolio-accounts
 * IdP and write the returned `sub` back to `User.idpSub`.
 *
  ...
  for (const u of users) {
    ...
    try {
      const res = await importUser({
        ...
        // Pro detection: Clara stored Pro state implicitly via dailyAgentMessageLimit > 30.
        plan: u.dailyAgentMessageLimit > 30 ? "pro" : "free",
        proUntil: u.subscriptionCurrentPeriodEnd?.toISOString() ?? undefined,
      });
