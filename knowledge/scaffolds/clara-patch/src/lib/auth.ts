...
/**
 * Clara auth, after IdP cutover.
 *
 * One OIDC provider, the IdP at user.trefolio.com. Local password / Google /
 * passkey providers have been removed; users sign up and manage credentials
 * on the IdP only.
 *
 * The `signIn` callback writes through to local `User` columns so the rest
 * of Clara (chat quota, agent flows, Telegram lookup) keeps working without
 * a network round-trip on every request.
 *
 * See knowledge/design-docs/clara-idp-integration.md (in trefolio repo).
 */
...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    async signIn({ user: _user, profile }) {
      ...
      const limit = p.entitlements?.clara_daily_limit ?? FREE_DAILY_LIMIT;
      // Lazy-create or update the local User keyed by IdP sub.
