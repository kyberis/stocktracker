...
- Mover selection: [`src/lib/widget/pick-top-movers.ts`](../../src/lib/widget/pick-top-movers.ts) — prefer 2 gainers + 1 loser by day %, fill from absolute movers if a side is empty, then sort by `|dayChange|` desc.
...
- Medium widget size recommended for movers (three rows).
