...
- **Rate limiting on signup/login** — 5 signups/IP/hour, 10 logins/IP/15 min via Upstash Redis (Turso fallback)
- **Cloudflare Turnstile CAPTCHA** — blocks bot signups; optional in dev (set `NEXT_PUBLIC_TURNSTILE_SITE_KEY` + `TURNSTILE_SECRET_KEY`)
- **MAX_PRO_SUBSCRIBERS raised to 500**
- **Analytics retention** — events older than 90 days purged daily via cron
- **Global OpenAI monthly cap** — 10,000 calls/month across all users; blocks with 429 when exhausted
- **Security headers** — HSTS + Content-Security-Policy added to all responses
- **Session secret enforced** — app throws in production if `APP_SESSION_SECRET` is unset
- **Stripe production mode** — live keys, webhook, Price IDs, and billing portal configured. E2E test added (`e2e/billing-checkout.spec.ts`) using test card `4242 4242 4242 4242`.
- **Domain & DNS** — `trefolio.com` purchased, DNS pointed to Vercel via Cloudflare, `APP_BASE_URL` set, Google OAuth redirect configured, Resend domain verified.
- **Cloudflare Turnstile setup** — Turnstile site created, `NEXT_PUBLIC_TURNSTILE_SITE_KEY` and `TURNSTILE_SECRET_KEY` configured in Vercel.
