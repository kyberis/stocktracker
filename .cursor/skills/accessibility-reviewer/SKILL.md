...
- Use Tailwind's `focus-visible:ring-2 focus-visible:ring-emerald-500` pattern consistent with the design system.

### 4) Color & Contrast

- Text must meet minimum contrast ratios: 4.5:1 for normal text, 3:1 for large text (≥18px bold or ≥24px).
- Verify both light and dark themes — check `dark:` variant pairings against the design system in `.cursor/rules/ui-design-system.mdc`.
...
- Route changes in Next.js should announce the new page title to assistive tech.
...
- Work with `engineer-dashboard` on chart and data-table accessibility.
- Work with `product-manager` on design review accessibility notes.
- Work with `legal-advisor` on regulatory accessibility obligations (EU Accessibility Act 2025).
- Work with `qa-tester` to add accessibility test cases.
