...
interface Props {
  ...
  /**
   * Controls whether the Recharts graph is rendered. When `false`, the card shows
   * only the header + a "deep dive" CTA and no history/benchmark requests are made.
   * Defaults to `true` when the prop is not provided, preserving the original
   * behaviour for callers like `/portfolio`.
   */
...
export default function PortfolioValueChart({ holdings, assetFilter, refreshKey, onRecalculate, recalculating, onOpenAi, expanded, onToggleExpand, totalValue, dayGainLoss, dayGainLossPercent, onAssetFilterChange, dayChangePctByType, chartVisible, onToggleChartVisible }: Props) {
  ...
  // When the prop is omitted (e.g. /portfolio route) we default to visible to
  // preserve existing behaviour. The home dashboards pass `false` to hide by
  // default until the user opens the deep dive CTA.
  const isChartVisible = chartVisible !== false;
  ...
  useEffect(() => {
    ...
    if (!isChartVisible) {
      ...
      // the collapsed CTA renders immediately.
      setLoading(false);
      return;
    }
  ...
  // Top-right controls for the collapsed (chart hidden) state. We intentionally
