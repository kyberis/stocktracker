...
function BusinessBlock({ card }: { card: ScreeningCandidateCard }) {
  ...
  return (
    <div className="mt-3 rounded-xl border border-[color:var(--border)] bg-[color:var(--surface-soft)] p-3">
      ...
      {facts.length > 0 && (
        <p className="mt-2 text-xs text-[color:var(--muted)]">{facts.join(" · ")}</p>
      )}
...
export function CandidateCard({ card }: { card: ScreeningCandidateCard }) {
  const { copy } = useScreeningCopy();
  ...
  const meta: Array<{ label: string; value: string }> = [
    { label: copy.report.metaPrice, value: price(card.price, card.currency) },
    {
  ...
  return (
    <article className="card rounded-[20px] border border-[color:var(--border)] p-4 sm:p-5">
      ...
      <dl className="mt-3 grid grid-cols-2 gap-1.5 sm:grid-cols-4">
        {meta.map((item) => (
          ...
          >
            ...
            <dd className="mt-0.5 text-sm font-semibold tabular-nums text-[color:var(--foreground)]">
              {item.value}
            </dd>
          </div>
        ))}
      ...
      <p className="mt-1.5 text-sm leading-relaxed text-[color:var(--foreground)]">{card.thesis}</p>
