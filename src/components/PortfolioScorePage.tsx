      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold flex items-center gap-3">
            ...
            {t("portfolioScoreToolTitle")}
          </h1>
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">{t("portfolioScoreToolSubtitle")}</p>
        </div>
        <BlurredProSection blurb={t("blurScoreTeaser")}>
          <div className="flex flex-col items-center gap-4 py-6">
            <svg viewBox="0 0 140 140" width="140" height="140">
              <circle cx="70" cy="70" r="60" fill="none" stroke="currentColor" strokeWidth="8" className="text-gray-100 dark:text-white/[0.06]" />
              <circle cx="70" cy="70" r="60" fill="none" stroke="currentColor" strokeWidth="8" strokeDasharray="280 377" strokeLinecap="round" className="text-emerald-500" transform="rotate(-90 70 70)" />
              <text x="70" y="64" textAnchor="middle" className="fill-emerald-500 text-3xl font-bold font-mono" fontSize="32">74</text>
              <text x="70" y="82" textAnchor="middle" className="fill-gray-400 dark:fill-slate-500 text-xs" fontSize="11">/ 100</text>
            </svg>
            <span className="text-sm font-semibold text-emerald-500">Good &mdash; Room for improvement</span>
            <div className="grid grid-cols-2 gap-2 w-full max-w-sm">
              {[
                { label: "Diversification", pct: 82, color: "bg-emerald-500" },
                { label: "Risk", pct: 65, color: "bg-amber-500" },
                { label: "Income", pct: 58, color: "bg-amber-500" },
                { label: "Growth", pct: 91, color: "bg-emerald-500" },
              ].map((c) => (
                <div key={c.label} className="p-2.5 rounded-lg border border-gray-200 dark:border-slate-700">
                  <div className="text-[10px] uppercase tracking-wider text-gray-400 dark:text-slate-500 mb-1">{c.label}</div>
        ...
        <div className="mt-6">
          <ProCompareCard surface="portfolio_score_locked" reason="upgrade_required" />
        </div>
      </div>
    );
  }
