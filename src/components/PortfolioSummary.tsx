...
export default function PortfolioSummary({ holdings: holdingsProp, cashEntries: cashEntriesProp, allCashEntries: allCashEntriesProp }: Props) {
  ...
  const netWorthBreakdown = useMemo<BreakdownItem[]>(() => {
    ...
    const fixedSlice = netWorthAllocationSlices.find((s) => s.key === "fixed_return");
  ...
  const aiReviewBtn = !aiReportEnabled ? null : (
    <button
      onClick={() => {
    ...
    >
      ...
      AI Review
      {!isAdmin && <span className="tabular-nums text-gray-400 dark:text-slate-500">{used}/{limit}</span>}
    </button>
  );
