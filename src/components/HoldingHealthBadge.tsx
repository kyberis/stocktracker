...
function HoldingHealthBadge({ holding, size = 32 }: Props) {
  ...
  if (!isPro) {
    return (
      <a
        href="/billing"
        className="relative inline-flex items-center justify-center rounded-full opacity-60 hover:opacity-80 transition-opacity"
        style={{ width: size, height: size }}
        title="Upgrade to Pro to see health scores"
        aria-label="Health score — Pro feature"
      >
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} aria-hidden="true">
          <circle cx={size / 2} cy={size / 2} r={(size - 4) / 2} fill="none" stroke="var(--border)" strokeWidth="2.5" opacity="0.3" />
        </svg>
        <svg className="absolute w-3 h-3 text-gray-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
