...
interface PageProps {
  ...
  searchParams: Promise<{ exchange?: string; from?: string }>;
}

export default async function CompanyAnalysisTickerPage({ params, searchParams }: PageProps) {
  const [{ ticker: raw }, { exchange, from }] = await Promise.all([params, searchParams]);
  ...
  return <AnalisisShell ticker={ticker} exchange={exchange || ""} from={from} />;
}
