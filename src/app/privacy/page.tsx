        ...
        <div className="prose prose-slate max-w-none space-y-10 [&_h2]:text-2xl [&_h2]:font-semibold [&_h2]:text-slate-900 [&_h2]:mt-12 [&_h2]:mb-4 [&_h3]:text-lg [&_h3]:font-medium [&_h3]:text-slate-800 [&_h3]:mt-8 [&_h3]:mb-3 [&_p]:leading-relaxed [&_li]:leading-relaxed">
          ...
          <section>
            ...
            <ul className="list-disc pl-6 space-y-2">
              <li>
                ...
                submission, a broker request, or a portfolio data-quality anomaly flagged for
                staff review) so our team can respond quickly. The same linked
                staff bot may also answer operational queries requested by our team
                in natural language or via short commands (for example the latest signup, latest feedback entries, the latest tracked
                product interaction, or aggregate experiment assignment counts per treatment) using only the minimum details needed plus a secure admin link,
                and may offer staff-only action buttons (acknowledge, apply safe data fixes, or
                dismiss) for portfolio anomaly alerts. When a staff question is not an exact command, we may send that question text (and experiment keys/names — not assignment user ids) to OpenAI solely to choose which operational query to run; the numeric answer is read from our database.
              </li>
              <li>
          ...
          <section>
            ...
            <ul className="list-disc pl-6 space-y-2">
              ...
              <li>Transcribe voice notes you send to Warren on Telegram (audio is forwarded to OpenAI for speech-to-text and discarded immediately after; only the transcript is retained). Warren&apos;s spoken replies are generated by OpenAI text-to-speech and sent back through Telegram (raw audio not stored).</li>
              <li>
                Answer optional chart Q&amp;A (your question and the chart snapshot — values, invested capital, time range, and trade markers — are sent to OpenAI; see Section 5). Explain optional performance-matrix cell figures (period formula inputs, attributed buy/sell transactions for that sleeve, and related totals — sent to OpenAI; see Section 5). When you ask Warren about a highlighted Holdings Explorer cell, we send your question plus the selected ticker, metric, displayed value, and sibling row figures (see Section 5). When Warren looks up public web results, investor-relations documents, or earnings-call excerpts to answer you, we send the ticker and a short query to search providers and then send those public excerpts to OpenAI (see Section 5).
              </li>
              ...
              <table className="w-full text-sm border-collapse">
                ...
                <tbody className="divide-y divide-slate-200">
                  ...
                  <tr>
                    ...
                    <td className="py-3 pr-6">AI-powered analysis, chart Q&amp;A, performance-matrix cell explanations, support chat, Warren voice (Telegram), Warren public-web / investor-relations / earnings-call research, Investor Briefing news digests, FinPulse social summaries (beta), company analysis (/analisis) narratives, Investment Screening research agents (experimental, flag-gated), holding classification Auto-fix (ticker, name, and related metadata sent to suggest sector, region, and asset class), import/portfolio data-quality summaries, staff-only portfolio anomaly explanations, and staff-only ProdOps query classification</td>
                    <td className="py-3 pr-6">
                      Portfolio data included in analysis prompts; optional Holdings Explorer cell questions (selected ticker, metric, displayed value, and sibling row figures); chart assistant questions with chart snapshot (values, invested capital, range, trades); support chat messages and optional portfolio summary; voice notes sent to Warren on Telegram (transcribed by OpenAI Whisper, audio not retained); Warren spoken replies generated by OpenAI text-to-speech (audio not retained server-side). When Warren searches the public web, investor-relations pages/PDFs, or earnings-call transcripts to answer a question, we send those public excerpts (plus your question) to OpenAI — not your email or name. When you use the optional Investor Briefing beta at /aid, we may send your portfolio tickers, recent news context, and excerpts from public X posts to generate short bullet summaries (including FinPulse). For company analysis (/analisis), we send structured market data for the ticker you request plus optional public web-search excerpts to generate short interpretive narratives (not personalized advice). When you use Investment Screening (/screening), we may send your screening brief (including optional risk profile: conservative / balanced / aggressive), candidate tickers, compact holdings/cash/sector snapshots, and public news/web/IR excerpts (including text extracted from official investor-relations HTML pages and PDFs) to generate research cards (AI-generated; may be inaccurate; not investment advice). When you use Auto-fix on Tools → Classification, we send the holding&apos;s ticker, name, ISIN, exchange, and asset-type hints to suggest sector, region, and asset class (not investment advice). When you import a broker file or run Audit &amp; repair on /import, we may send a compact list of data-quality findings (tickers, currencies, and rule codes — not your full portfolio) to generate a short explanatory summary (AI-generated; may be inaccurate; not investment advice). For staff operational review, we may send a compact list of portfolio anomaly findings (rule codes, tickers, and short evidence — not your full portfolio) to generate a staff-only explanation and remediation prompt (AI-generated; may be inaccurate; not investment advice; not shown to end users). For the staff ProdOps Telegram bot, we may send the operator&apos;s question text plus experiment keys and names (not assignment user ids or other account PII) to classify which operational query to run; counts and user summaries are then read from our database, not generated by the model.
                    </td>
                  </tr>
                  <tr>
                    <td className="py-3 pr-6">Tavily</td>
                    <td className="py-3 pr-6">Web search for Investor Briefing earnings-day news digests, FinPulse X post discovery (beta, optional), company analysis (/analisis) enrichment, Investment Screening Web &amp; Sentiment plus optional Company Research and IR document extract (experimental, flag-gated), and Warren answers that need public web, investor-relations, or earnings-call context</td>
                    <td className="py-3">Stock ticker symbols, company names, curated X handles, and search queries; for screening and Warren research, only ticker and company name (plus a short query for Warren) are sent — no account email or name. Screening and Warren may also send public IR page and filing URLs (including PDFs) to Tavily Extract.</td>
                  </tr>
                  <tr>
                    <td className="py-3 pr-6">Serper</td>
                    <td className="py-3 pr-6">Optional Investment Screening IR document discovery (experimental, flag-gated) and Warren investor-relations lookups: Google search for official investor-relations pages</td>
                  ...
                  <tr>
                    ...
                    <td className="py-3 pr-6">Optional Investment Screening IR document extract (experimental, flag-gated) and Warren investor-relations lookups: public HTML pages and PDFs from official investor-relations sites</td>
                  ...
                  <tr>
                    ...
                    <td className="py-3 pr-6">Optional recent note tags on Investor Briefing when you use the same trefolio account</td>
                    <td className="py-3 pr-6">Server-to-server request with your IdP subject identifier; returns recent tag labels and a short excerpt from your notes when you have a linked Will account</td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td className="py-3 pr-6">Market data (Trefolio), including optional MCP proxy and Warren earnings-call transcript excerpts</td>
                    <td className="py-3">
                  ...
                  <tr>
                    ...
                    <td className="py-3">Telegram chat id for outbound price alerts; messages contain stock ticker and price data only. For staff operational alerts, we also store the linked recipient&apos;s Telegram user id, username, optional display name, and DM chat id after an admin completes the Telegram Start link flow. Staff-operated operational channels may receive concise summaries of signup, membership, feedback, broker-request, trial-activation, or portfolio-anomaly events with an admin link and no full free-text feedback body or full holdings dump. The same linked staff bot may answer operational queries such as the latest signup, latest feedback entries, the latest tracked product interaction, or aggregate experiment assignment counts per treatment using minimal account details only, including natural-language questions from the linked operator, and may present staff-only buttons to acknowledge, apply safe data repairs, or dismiss a portfolio anomaly.</td>
                  </tr>
                  <tr>
                    <td className="py-3 pr-6">Telegram Messenger Inc.</td>
                    <td className="py-3 pr-6">Optional Warren chat over Telegram (text and voice)</td>
                    <td className="py-3">If you choose to connect your Telegram account, we exchange your Telegram chat ID and the contents of messages you send to our bot with Telegram&apos;s servers; the same messages are then sent to OpenAI to generate Warren&apos;s reply. If you send a voice note, the audio is downloaded from Telegram, forwarded to OpenAI for speech-to-text transcription (Whisper), and discarded immediately after. Warren&apos;s spoken reply is generated by OpenAI text-to-speech and sent back through Telegram; we do not retain raw audio. You can disconnect at any time from your profile.</td>
                  </tr>
                  <tr>
