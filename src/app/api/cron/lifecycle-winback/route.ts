...
// No dedicated "win-back" copy exists in email-i18n — content there is
// scoped to onboarding/upgrade/feature templates. AI analysis is trefolio's
// clearest differentiator (per the competitive review), so it doubles as
// the re-engagement nudge for v1. Swap the slug to any other feature-*
// template if a better fit turns up later; the send/dedupe logic is unchanged.
const TEMPLATE_SLUG = "feature-ai-analysis";

const runLifecycleWinback = withCronLogging("lifecycle-winback", async () => {
  ...
  // Users inactive 14+ days who do have holdings (an activated user gone
  // quiet, not a signup that never activated — that's the activation cron's
  // job) and haven't already received this template.
