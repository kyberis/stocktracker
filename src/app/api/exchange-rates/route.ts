...
import { getCachedRates, setCachedRate } from "@/lib/quote-cache";
...
export const GET = withMetrics("/api/exchange-rates", async (request: NextRequest) => {
  ...
  for (const [pair, rate] of Object.entries(hits)) {
    results[pair] = { rate, provider: "yahoo" };
  }

  if (misses.length > 0) {
    const tasks = misses.map(async (pair) => {
