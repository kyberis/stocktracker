...
describe("useSnapTradeApi fetchBroker", () => {
  it("fetchBroker success sets transactions and step to preview", async () => {
    fetchSpy.mockResolvedValueOnce({
      ok: true,
      json: () =>
        Promise.resolve({
    ...
    const { result } = renderHook(() => useSnapTradeApi());
    await act(async () => {
      await result.current.fetchBroker("broker-conn-1");
    });

    expect(result.current.step).toBe("preview");
    expect(result.current.transactions).toHaveLength(1);
    expect(result.current.transactions[0].ticker).toBe("GOOG");
    expect(result.current.syncingBrokerId).toBeNull();
  });

  it("fetchBroker when res not ok sets error and step", async () => {
    fetchSpy.mockResolvedValueOnce({
      ok: false,
      json: () => Promise.resolve({ error: "Broker fetch failed" }),
    });

    const { result } = renderHook(() => useSnapTradeApi());
    await act(async () => {
      await result.current.fetchBroker("broker-1");
    });

    expect(result.current.step).toBe("error");
    expect(result.current.errorMsg).toBe("Broker fetch failed");
  });

  it("fetchBroker when fetch throws sets error and step", async () => {
    fetchSpy.mockRejectedValueOnce(new Error("Network error"));
  ...
  it("fetchBroker sends brokerConnectionId, portfolioId, startDate in FormData", async () => {
    fetchSpy.mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ transactions: [] }),
    });
    ...
    const { result } = renderHook(() => useSnapTradeApi());
    await act(async () => {
      await result.current.fetchBroker("broker-xyz", "port-2", "2024-02-15");
    });

    const form = fetchSpy.mock.calls[0][1]?.body as FormData;
    expect(form.get("action")).toBe("fetch");
    expect(form.get("brokerConnectionId")).toBe("broker-xyz");
    expect(form.get("portfolioId")).toBe("port-2");
    expect(form.get("startDate")).toBe("2024-02-15");
  });
});
